...
pub trait ThunkImpl {
  fn apply(&self, _ctr: &CtxCtr, /*_env: &CtxEnv,*/ _spec_: &dyn ThunkSpec_, _param: &[ScalarVal_], _args: &[(CellPtr, Clock)], _th: ThunkPtr, _out: CellPtr, _prev_oclk: Clock, _oclk: Clock) -> ThunkResult { ThunkErr::NotImpl.into() }
  fn accumulate(&self, _ctr: &CtxCtr, /*_env: &CtxEnv,*/ _spec_: &dyn ThunkSpec_, _param: &[ScalarVal_], _arg: &[(CellPtr, Clock)], _th: ThunkPtr, _out: CellPtr, _prev_oclk: Clock, _oclk: Clock) -> ThunkResult { ThunkErr::NotImpl.into() }
  fn initialize(&self, _ctr: &CtxCtr, /*_env: &CtxEnv,*/ _spec_: &dyn ThunkSpec_, _param: &[ScalarVal_], _arg: &[(CellPtr, Clock)], _th: ThunkPtr, _out: CellPtr, _prev_oclk: Clock, _oclk: Clock) -> ThunkResult { ThunkErr::NotImpl.into() }
}

pub trait ThunkImpl_ {
  fn as_any(&self) -> &dyn Any;
  fn apply(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], args: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock) -> ThunkResult;
  fn accumulate(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock) -> ThunkResult;
  fn initialize(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock) -> ThunkResult;
}
...
impl<T: ThunkImpl + Any> ThunkImpl_ for T {
  ...
  fn apply(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock) -> ThunkResult {
    ThunkImpl::apply(self, ctr, /*env,*/ spec_, param, arg, th, out, prev_oclk, oclk)
  }

  fn accumulate(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock) -> ThunkResult {
    ThunkImpl::accumulate(self, ctr, /*env,*/ spec_, param, arg, th, out, prev_oclk, oclk)
  }

  fn initialize(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock) -> ThunkResult {
    ThunkImpl::initialize(self, ctr, /*env,*/ spec_, param, arg, th, out, prev_oclk, oclk)
  }
}
...
impl<T: FutharkThunkSpec> ThunkSpec for T {
  ...
  fn gen_impl_(&self, spec_dim: Vec<Dim>, pmach: PMach) -> Option<Rc<dyn ThunkImpl_>> {
    ...
    let param_ct = FutharkThunkSpec::param_count(self);
    let code = match FutharkThunkSpec::gen_futhark(self, arg_dim, out_dim) {
      Err(e) => {
    ...
    assert_eq!(code.abi.param_ct, param_ct);
  ...
  fn pop_adj(&self, arg: &[(CellPtr, Clock)], out: CellPtr, out_clk: Clock, out_mode: ThunkMode, out_adj: CellPtr, arg_adj: &mut [CellPtr]) -> Result<(), ThunkAdjErr> {
    ...
    match FutharkThunkSpec::pop_adj(self, arg, out, out_clk, out_adj, arg_adj) {
      Ok(FutharkThunkAdj::Auto) => {
        ...
        let primal_param_ct = FutharkThunkSpec::param_count(self);
        ...
        */
        let mut code = match self.gen_futhark(arg_dim, out_dim) {
          Err(e) => panic!("ERROR: failed to generate futhark thunk code: {:?}", e),
          Ok(code) => code
        };
        assert_eq!(code.abi.param_ct, primal_param_ct);
        ...
        let (primal_genabi, primal_source) = code.gen_source(primal_lar, primal_rar, primal_param_ct, &dim, out_mode, FutharkThunkGenConfig::default(), code.abi.clone())
...
impl FutharkThunkGenCode {
  ...
  pub fn gen_source(&self, lar: u16, rar: u16, param_ct: u16, spec_dim: &[Dim], mode: ThunkMode, mut cfg: FutharkThunkGenConfig, mut genabi: FutharkThunkGenAbi) -> Result<(FutharkThunkGenAbi, Vec<String>), ()> {
    let mut warn = false;
    ...
    for k in 0 .. param_ct {
      pats.push(format!(r"{{%param[{}]}}", k));
      reps.push(format!(r"param_{}", k));
    }
    for k in 0 .. lar {
      ...
      match mode {
        ...
        ThunkMode::Initialize => {
          ...
          if dim.ndim >= 1 && !cfg.emit_out0_shape {
            ...
            return self.gen_source(lar, rar, param_ct, spec_dim, mode, cfg, genabi);
          }
    ...
    match mode {
      ThunkMode::Accumulate => {
        ...
        genabi.arityin += 1;
        genabi.set_arg(lar, genabi.get_out(0));
      }
      ThunkMode::Initialize => {
        let arityin = genabi.arityin;
        assert_eq!(arityin, lar + 1);
        assert_eq!(genabi.get_arg(lar), genabi.get_out(0));
      }
      _ => {}
    }
    if cfg.emit_out0_shape_param {
...
pub trait FutharkThunkImpl_<B: FutBackend> {
  ...
  fn _build_object(ctr: &CtxCtr, /*env: &CtxEnv,*/ config: &FutConfig, name: Option<&str>, source: &[String], rst: Counter) -> Option<(FutObject<B>, Vec<(PAddr, StableCell)>)>;
}
...
impl FutharkThunkImpl_<MulticoreBackend> for FutharkThunkImpl<MulticoreBackend> {
  ...
  fn _build_object(ctr: &CtxCtr, /*env: &CtxEnv,*/ config: &FutConfig, name: Option<&str>, source: &[String], rst: Counter) -> Option<(FutObject<MulticoreBackend>, Vec<(PAddr, StableCell)>)> {
    // FIXME FIXME
      ...
      Ok(Some(mut obj)) => {
        ...
        for p in (pstart.to_unchecked() ..= pfin.to_unchecked()) {
          ...
          TL_PCTX.with(|pctx| {
            ...
            pcel.push(x, xclk, locus, pmach, p);
            TL_CTX.with(|ctx| {
              ctx.env.borrow_mut().insert_phy(x, ty, pcel);
            });
          });
          TL_CTX.with(|ctx| {
...
impl FutharkThunkImpl_<CudaBackend> for FutharkThunkImpl<CudaBackend> {
  ...
  fn _build_object(ctr: &CtxCtr, /*env: &CtxEnv,*/ config: &FutConfig, name: Option<&str>, source: &[String], rst: Counter) -> Option<(FutObject<CudaBackend>, Vec<(PAddr, StableCell)>)> {
...
impl<B: FutBackend> FutharkThunkImpl<B> where FutharkThunkImpl<B>: FutharkThunkImpl_<B> {
  pub fn _try_build(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ mode: ThunkMode, rst: Counter) {
...
impl FutharkThunkImpl<MulticoreBackend> {
  pub fn _enter(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock, mode: ThunkMode) -> ThunkResult {
    ...
    'for_k: for k in 0 .. lar {
      ...
      for j in 0 .. k as usize {
        match ctx.env.borrow()._lookup_view(arg[j].0) {
          Err(_) => panic!("bug"),
          Ok(e_j) => {
            let root_j = e_j.root();
            if root_j == root {
              if _cfg_debug_mode(mode) {
              println!("DEBUG: FutharkThunkImpl::<MulticoreBackend>::_enter: aliased args: root[{}]={:?} root[{}]={:?}",
                  j, root_j, k, root);
              }
              if &arg_ty_[j] == e.ty && arg[j].1 == arg[k as usize].1 {
                arg_ty_.push(arg_ty_[j].clone());
                let a = arg_arr[j].get_mut().clone();
                arg_arr.push(a.into());
              } else {
                println!("DEBUG: FutharkThunkImpl::<MulticoreBackend>::_enter: aliased args: root[{}]={:?} {:?} root[{}]={:?} {:?}",
                    j, root_j, arg[j].1, k, root, arg[k as usize].1);
      ...
      let v_ty = match e.view().eval_contiguous(e.root_ty) {
        Err(_) => {
          println!("ERROR: FutharkThunkImpl::<MulticoreBackend>::_enter: arg is not a zero-copy (contiguous) view");
          panic!();
        }
        Ok(ty) => ty
      };
    ...
    let mut out_org = Vec::with_capacity(genabi.arityout as usize);
    for k in 0 .. genabi.arityout {
      assert_eq!(k, 0);
      let ty_ = match mode {
        ThunkMode::Apply => {
        ...
        ThunkMode::Initialize => {
          ...
          let env = ctx.env.borrow();
          let e = match env.prewrite_view(out, prev_oclk, oclk, Locus::Mem) {
    ...
    let mut param_buf: Vec<FutAbiScalar> = Vec::with_capacity(np);
    param_buf.resize(np, FutAbiScalar::Unspec);
    {
      let mut spec_ct = 0;
      let mut shape_idx = 0;
      ...
      for pidx in 0 .. np {
        match genabi.get_param(pidx as _) {
          (FutharkParam::Spec, sty) => {
            let val = param[pidx].to_abi_scalar();
            assert_eq!(sty, val.type_());
            param_buf[pidx] = val;
            spec_ct += 1;
          }
          (FutharkParam::Out0Shape, FutAbiScalarType::I64) => {
            match genabi.get_out(0) {
              FutharkArrayRepr::Nd => {
                param_buf[pidx] = FutAbiScalar::I64(out_ty_[0].shape[shape_idx].into());
                shape_idx += 1;
              }
              FutharkArrayRepr::Flat => {
                assert!(!flat_out0_shape);
                param_buf[pidx] = FutAbiScalar::I64(out_ty_[0].flat_len().into());
    ...
    let o_ret = obj.enter_kernel(eabi, &param_buf, &arg_arr, &out_arr);
    if o_ret.is_err() {
    ...
    match mode {
      ThunkMode::Accumulate |
      ThunkMode::Initialize => {
        assert!(out_org[0].is_some());
        let (out_org_addr, out_org_ptr, out_org_size) = out_org[0].unwrap();
        if out_org_ptr != out_ptr {
          if _cfg_debug_mode(mode) {
          println!("WARNING: FutharkThunkImpl::<MulticoreBackend>::_enter: out: {:?} not in-place!", mode);
    ...
    TL_PCTX.with(|pctx| {
      ...
      if _cfg_debug_mode(mode) {
      println!("DEBUG: FutharkThunkImpl::<MulticoreBackend>::_enter: out: ptr=0x{:016x} addr={:?} root={:?}",
          out_ptr as usize, addr, pctx.lookup_root(addr));
      }
      let mut f = false;
      if !f {
        for k in objects.find(mode).unwrap().1.consts.iter() {
          if k.0 == addr {
            ...
            match ctx.env.borrow_mut().celtab.get_mut(&k.1) {
              None => panic!("bug"),
              Some(e) => {
                if e.ty.is_top() {
                  if _cfg_debug_mode(mode) { println!("DEBUG: FutharkThunkImpl::<MulticoreBackend>::_enter: out: const {:?} : top = {:?}", k.1, root_ty); }
                  e.ty = root_ty;
                } else {
                  if _cfg_debug_mode(mode) { println!("DEBUG: FutharkThunkImpl::<MulticoreBackend>::_enter: out: const {:?} : {:?} ?= {:?}", k.1, &e.ty, &root_ty); }
                  assert_eq!(&e.ty, &root_ty);
                }
              }
            }
            break;
            ...
            match &mut *cel_ {
              ...
              &mut Cell_::Phy(ref state, .., ref mut pcel) => {
                ...
                if e.root_ty.span_bytes() != e.ty.span_bytes() {
                  ...
                  let optr = pcel.optr;
                  // FIXME: pmach.
                  match pcel.swap(optr, oclk, Locus::Mem, PMach::NvGpu, addr) {
                    None => {}
                    Some((_, prev_addr)) => {
                      if _cfg_debug_mode(mode) {
...
impl FutharkThunkImpl<CudaBackend> {
  pub fn _enter(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ spec_: &dyn ThunkSpec_, param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock, mode: ThunkMode) -> ThunkResult {
    ...
    'for_k: for k in 0 .. lar {
      let xroot = match ctx.env.borrow()._lookup_view(arg[k as usize].0) {
        Err(_) => panic!("bug"),
        Ok(e) => e.root()
      };
      for j in 0 .. k as usize {
        match ctx.env.borrow()._lookup_view(arg[j].0) {
          ...
          Ok(e_j) => {
            ...
            if xroot_j == xroot {
              ...
              match ctx.env.borrow()._lookup_view(arg[k as usize].0) {
                Err(_) => panic!("bug"),
                Ok(e) => {
                  if &arg_ty_[j] == e.ty {
                    arg_ty_.push(arg_ty_[j].clone());
      ...
      TL_PCTX.with(|pctx| {
        ...
        let v_ty = match e.view().eval_contiguous(e.root_ty) {
          Err(_) => {
            println!("ERROR: FutharkThunkImpl::<CudaBackend>::_enter: arg is not a zero-copy (contiguous) view");
            panic!();
          }
          Ok(ty) => ty
        };
        ...
        match &mut *cel_ {
          &mut Cell_::Phy(.., ref mut pcel) => {
            ...
            let sz = e.ty.span_bytes() as usize;
            assert!(is_subregion_dev(dptr, sz, base, base_sz));
            a.set_mem_parts(dptr, sz);
          }
          _ => panic!("bug")
        }
        if self.spec_dim[k as usize].ndim == 0 {
      ...
      let ty_ = match mode {
        ...
        ThunkMode::Initialize => {
          TL_PCTX.with(|pctx| {
            ...
            // FIXME: double check that out does not alias any args.
            let env = ctx.env.borrow();
            let e = match env.prewrite_view(out, prev_oclk, oclk, loc) {
              /*Err(CellDerefErr::View) => {
    ...
    obj.release();
    let post_front_dptr = TL_PCTX.with(|pctx| {
      let gpu = pctx.nvgpu.as_ref().unwrap();
      gpu.compute.sync().unwrap();
            ...
            for k in objects.find(mode).unwrap().1.consts.iter() {
              if k.0 == addr {
                ...
                match ctx.env.borrow_mut().celtab.get_mut(&k.1) {
                  ...
                  Some(e) => {
                    if e.ty.is_top() {
                      if _cfg_debug_mode(mode) { println!("DEBUG: FutharkThunkImpl::<CudaBackend>::_enter: out: const {:?} : top = {:?}", k.1, root_ty); }
                      e.ty = root_ty;
                    } else {
                      if _cfg_debug_mode(mode) { println!("DEBUG: FutharkThunkImpl::<CudaBackend>::_enter: out: const {:?} : {:?} ?= {:?}", k.1, &e.ty, &root_ty); }
                      assert_eq!(&e.ty, &root_ty);
                    }
                  }
                }
...
impl PThunk {
  ...
  pub fn apply(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock, pmach: PMach) -> ThunkResult {
    ...
    match self.lookup_impl_(pmach) {
      None => panic!("bug"),
      Some(thimpl_) => {
        thimpl_.apply(ctr, /*env,*/ &*self.spec_, param, arg, th, out, prev_oclk, oclk)
      }
    }
  }

  pub fn accumulate(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock, pmach: PMach) -> ThunkResult {
    //let primary = TL_CTX.with(|ctx| ctx.primary.get());
    //let pmach = primary.unwrap_or_else(|| TL_PCTX.with(|pctx| pctx.fastest_pmach()));
    ...
    match self.lookup_impl_(pmach) {
      None => panic!("bug"),
      Some(thimpl_) => {
        thimpl_.accumulate(ctr, /*env,*/ &*self.spec_, param, arg, th, out, prev_oclk, oclk)
      }
    }
  }

  pub fn initialize(&self, ctr: &CtxCtr, /*env: &CtxEnv,*/ param: &[ScalarVal_], arg: &[(CellPtr, Clock)], th: ThunkPtr, out: CellPtr, prev_oclk: Clock, oclk: Clock, pmach: PMach) -> ThunkResult {
    //let primary = TL_CTX.with(|ctx| ctx.primary.get());
    //let pmach = primary.unwrap_or_else(|| TL_PCTX.with(|pctx| pctx.fastest_pmach()));
    ...
    match self.lookup_impl_(pmach) {
      None => panic!("bug"),
      Some(thimpl_) => {
        thimpl_.initialize(ctr, /*env,*/ &*self.spec_, param, arg, th, out, prev_oclk, oclk)
      }
    }
  }
}
